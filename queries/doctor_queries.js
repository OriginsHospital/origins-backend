...
WITH appointments as (
    SELECT
        ...
        pm.patientId,
        caa.branchId,
        pm.branchId as homeBranchId,
        (SELECT bm.branchCode FROM branch_master bm WHERE bm.id = pm.branchId) AS homeBranchCode,
        (SELECT bm.name FROM branch_master bm WHERE bm.id = pm.branchId) AS homeBranchName,
        (SELECT bm.branchCode FROM branch_master bm WHERE bm.id = caa.branchId) AS appointmentBranchCode,
        caa.appointmentType,
    ...
    SELECT
        ...
        (SELECT bm.branchCode FROM branch_master bm WHERE bm.id = pm.branchId) AS homeBranchCode,
        (SELECT bm.name FROM branch_master bm WHERE bm.id = pm.branchId) AS homeBranchName,
        (SELECT bm.branchCode FROM branch_master bm WHERE bm.id = taa.branchId) AS appointmentBranchCode,
        taa.appointmentType,
        COALESCE(arm.name,'') as appointmentReason,
        arm.isSpouse,
        taa.stage,
